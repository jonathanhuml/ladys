...
sm_fdiff = lambda tau, l, w: np.exp(-1 / 2 * (tau / l) ** 2) * np.sin(w * tau * 2 * np.pi) + np.exp(
    -1 / 2 * (w * l * 2 * np.pi) ** 2) * imvoigt(tau / (l * np.sqrt(2)), (w * l * 2 * np.pi) / np.sqrt(2))

### EXPONENTIATED QUADRATIC ###

sq_fplus = lambda tau, l: np.exp(-1 / 2 * (tau / l) ** 2)
sq_fdiff = lambda tau, l: (2 / np.sqrt(np.pi)) * special.dawsn((tau / (l * np.sqrt(2))))
...
def myqr_vjp(g, ans, x):
    ...
    # Instead of using inv(rt), solve the system rt.T * X
    X = solve(rt.T, (gr + mid * tmp).T).T

    # Similarly, solve rt.T * Y 
    Y = solve(rt.T, (gq - m(q, m(qt, gq))).T).T
