...
class EmbeddedLowDNetwork(DynamicalSystem):
    def __init__(self, low_d_system, net_size=64, base_rate=1.0, dt=0.01):
        ...
        self.proj = (np.random.rand(self.low_d_system.net_size, self.net_size) + 1) * np.sign(
            np.random.randn(self.low_d_system.net_size, net_size))
...
class ChaoticNetwork(DynamicalSystem):
    ...
    def __init__(self, num_inits=100, base_rate=1.0, net_size=64, weight_scale=5.0, dt=0.01, inputs=None):
        ...
        self.net_size = net_size
        self.bias = np.log(base_rate)
        self.proj = (np.random.rand(self.net_size, self.net_size) + 1) * np.sign(
            np.random.randn(self.net_size, net_size)) / np.sqrt(self.net_size)
...
class SyntheticCalciumDataGenerator():
    ...
    def generate_dataset(self):
        ...
        #         pdb.set_trace()
        calcium = self.calcium_dynamics.integrate(num_steps=self.num_steps,
                                                  inputs=spikes.transpose(2, 0, 1, 3)).transpose(1, 2, 0, 3)
...
def LorenzData(num_inits, neurons, num_trials, device, seed):
    ...
    train_latent = torch.Tensor(data_dict['train_latent']).to(device)
    valid_latent = torch.Tensor(data_dict['valid_latent']).to(device)
    ...
    return train_data, valid_data, train_truth, valid_truth, train_latent, valid_latent
